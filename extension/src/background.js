...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.type === 'FETCH_TTS') {
        ...
        fetchTTSWithTimeout(request.name, request.sounds_like, request.cultural_note, 25000)
...
async function fetchTTSWithTimeout(name, sounds_like, cultural_note, timeoutMs) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/tts`, {
            ...
            body: JSON.stringify({ name, sounds_like, cultural_note }),
